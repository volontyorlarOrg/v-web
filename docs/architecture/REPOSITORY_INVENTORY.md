...
| `src/app/icon.svg`, `icon.png`, `apple-icon.png`, `favicon.ico`   | Next.js metadata-file assets inherited inside the localized layout; copies of the logo kit's favicon files |

## Components

| Area                      | Files and boundary                                                                                                                                                                                                                                                    |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Brand                     | `brand/logo.tsx` owns the icon, wordmark, heart and lockup SVG markup; `brand/logo-paths.ts` holds the logo kit's path data; `brand/channel-icons.tsx` owns the Telegram, Instagram and LinkedIn glyphs                                                               |
...
The five files under `docs/brand/reference/` are superseded brand history:
`circle-arc-mark.svg` and `circle-arc-logo-spec.md` (the mark before the logo
kit), `volontyorlar-logo-reference.jpg`, `volontyor-legacy-mark.png`, and
`volontyor-legacy-mark-white.png`.

## Public brand asset package

`public/logo/` is the Volontyorlar web logo kit installed as a deliberate
export package, not dead application code. The site renders the logo inline,
while the manifest, structured data and external consumers use these files.
`public/opengraph-image.png` is the kit's 1200×630 social image, referenced by
absolute URL from localized page metadata.

- `svg/`: `icon-{blue,blue-orange-heart,orange,white-blue-glyph}.svg`,
  `glyph-{blue,blue-orange-heart,white,white-orange-heart,currentcolor}.svg`,
  `wordmark-{blue,blue-orange-heart,orange,orange-blue-heart,white,white-orange-heart,currentcolor}.svg`,
  `lockup-{blue,blue-orange-heart,orange-white,white,white-orange-heart}.svg`,
  `heart-{blue,orange,white,currentcolor}.svg`.
- `png/icon/`, `png/glyph/`, `png/heart/`, `png/wordmark/`, `png/lockup/`: the
  same artwork as transparent rasters, suffixed with their width (`-512w`).
- `favicon/`: `favicon.{ico,svg}`, `favicon-{16x16,32x32,48x48,96x96}.png`,
  `apple-touch-icon.png`, `android-chrome-{192x192,512x512}.png`,
  `maskable-icon-{192x192,512x512}.png`.
- `social/`: `og-image-1200x630.png`, `og-image-light-1200x630.png`,
  `avatar-{blue,orange,white}-1080.png`.
